...
@allure.story("Endpoint 2 Investigation")
class TestEndpoint2Discovery:
    """Comprehensive discovery tests for /api/test/2
    ...
    def test_endpoint_2_basic_get(self, base_url, headers):
        """Test basic GET request (expected to fail with 500)"""
        endpoint = f"{base_url}{TEST_ENDPOINT_2}"
        ...
        # Record results
        _endpoint_results[TEST_ENDPOINT_2]["status_codes"].append(response.status_code)
        _endpoint_results[TEST_ENDPOINT_2]["response_times"].append(response.elapsed.total_seconds())
        _endpoint_results[TEST_ENDPOINT_2]["test_count"] += 1
    ...
    def test_endpoint_2_with_query_parameters(self, base_url, headers, query_params, description):
        ...
        with allure.step("Verify expected failure"):
            if response.status_code == HTTP_OK:
                logger.warning(f"⚠️ UNEXPECTED SUCCESS with params: {query_params}")
                _endpoint_results[TEST_ENDPOINT_2]["passed"] += 1
                pytest.fail(f"Endpoint unexpectedly succeeded with params {query_params}")
            else:
                _endpoint_results[TEST_ENDPOINT_2]["failed"] += 1
    ...
    def test_endpoint_2_different_http_methods(self, base_url, headers, http_method, request_func, description):
    ...
    def test_endpoint_2_consistency_rapid_fire(self, base_url, headers, num_requests, description):
        ...
        for i in range(num_requests):
            ...
            # Record results
            _endpoint_results[TEST_ENDPOINT_2]["status_codes"].append(response.status_code)
            _endpoint_results[TEST_ENDPOINT_2]["response_times"].append(response_time)
            _endpoint_results[TEST_ENDPOINT_2]["test_count"] += 1
    ...
    def test_endpoint_2_consistency_with_delays(self, base_url, fresh_auth_token, num_requests, delay, description):
    ...
    def test_endpoint_2_with_different_tokens(self, base_url):
    ...
    def test_endpoint_2_additional_headers(self, base_url, headers, extra_headers, description):
    ...
    def test_endpoint_2_response_structure(self, base_url, headers):
    ...
    def test_endpoint_2_response_time_consistency(self, base_url, headers):
        ...
        for i in range(10):
            ...
            # Record results
            _endpoint_results[TEST_ENDPOINT_2]["status_codes"].append(response.status_code)
            _endpoint_results[TEST_ENDPOINT_2]["response_times"].append(elapsed)
            _endpoint_results[TEST_ENDPOINT_2]["test_count"] += 1
            _endpoint_results[TEST_ENDPOINT_2]["failed"] += 1
    ...
    def test_endpoint_2_redirect_detection(self, base_url, headers):
        ...
        logger.info(f"\n{'='*60}")
        logger.info(f"REDIRECT DETECTION TEST: {TEST_ENDPOINT_2}")
        logger.info(f"{'='*60}")
    ...
    def test_endpoint_2_after_sequence(self, base_url, fresh_auth_token, sequence):
        ...
        endpoint_2 = f"{base_url}{TEST_ENDPOINT_2}"
        ...
        _endpoint_results[TEST_ENDPOINT_2]["status_codes"].append(response_2.status_code)
        _endpoint_results[TEST_ENDPOINT_2]["response_times"].append(response_2.elapsed.total_seconds())
        _endpoint_results[TEST_ENDPOINT_2]["test_count"] += 1
    ...
    def test_endpoint_2_server_routing(self, base_url, headers):
    ...
    def test_endpoint_2_timing_analysis(self, base_url, headers):
    ...
    def test_endpoint_2_large_payload(self, base_url, headers):
        ...
        """
        endpoint = f"{base_url}{TEST_ENDPOINT_2}"
        large_payload = {"data": "x" * 10_000_000}  # 10 MB of data
