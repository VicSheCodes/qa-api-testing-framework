...
pytest tests/discovery/test_endpoint_1_discovery_allure.py \
  --alluredir=allure-results \
  -v

# Step 2: Generate standalone single-page HTML report
allure generate allure-results \
  --single-file \
  --clean \
  -o reports/allure

# Output: reports/allure/index.html (single file, fully self-contained)
"""
...
@allure.feature("Endpoint 1 - Advanced Discovery")
@allure.story("Endpoint Resilience Testing")
@pytest.mark.parametrize("test_endpoint", [TEST_ENDPOINT_1])
class TestEndpoint1AdvancedDiscovery:
    """Advanced discovery tests to find failure scenarios for endpoint 1"""

    @allure.title("Rapid-Fire Stress Test (100 requests)")
    @allure.description("Tests endpoint stability under rapid successive requests")
    ...
    def test_stress_rapid_fire(self, base_url, headers, test_endpoint):
        ...
        with allure.step(f"Execute 100 rapid requests to {test_endpoint}"):
            for i in range(100):
                try:
                    response = requests.get(
                        endpoint,
                    ...
                    if response.status_code == 429:
                        allure.attach(
                            f"Rate limit detected at request #{i + 1}",
                            name="Rate Limiting",
        ...
        failures = [r for r in results if r.get("status") != HTTP_OK]
        summary = f"Completed {len(results)} requests\n"
        summary += f"Failures: {len(failures)}\n"
        summary += f"Success Rate: {((len(results) - len(failures)) / len(results) * 100):.1f}%"

        allure.attach(
            summary,
            name="Rapid-Fire Test Summary",
            attachment_type=AttachmentType.TEXT
        )
    ...
    @allure.title("HTTP Method Variations")
    @allure.description("Tests endpoint behavior with different HTTP methods")
    ...
    def test_http_method_variations(self, base_url, headers, test_endpoint):
        ...
        with allure.step("Test each HTTP method"):
            for method_name, method_func in methods.items():
                with allure.step(f"Testing {method_name} method"):
                    response = method_func(
                        endpoint,
                        headers=headers,
                        verify=SSL_VERIFY,
                    ...
                    if response.status_code == expected_status:
                        _endpoint_results[test_endpoint]["passed"] += 1
                        logger.info(f"✓ {method_name} properly rejected: {response.status_code}")
                    elif response.status_code == HTTP_OK:
                        _endpoint_results[test_endpoint]["failed"] += 1
                        logger.warning(f"⚠️ {method_name} unexpectedly accepted: {response.status_code}")

                        # Attach warning to Allure
                        allure.attach(
                            f"{method_name} returned 200 OK instead of 405",
                            name=f"Unexpected {method_name} Acceptance",
                            attachment_type=AttachmentType.TEXT
                        )
                    else:
                        _endpoint_results[test_endpoint]["failed"] += 1
                        logger.warning(f"⚠️ {method_name} unexpected status: {response.status_code}")

    # Add similar decorators to other tests...